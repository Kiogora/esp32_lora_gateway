...
int ESP32_read(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
    /*Does it accept tv or ticks*/
...
int ESP32_write(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
    /*Does it accept tv or ticks*/
	TickType_t xTicksToWait = timeout_ms / portTICK_PERIOD_MS; /* convert milliseconds to ticks */
	TimeOut_t xTimeOut;
...
{
	n->my_socket = 0;
	n->mqttread = ESP32_read;
	n->mqttwrite = ESP32_write;
}
